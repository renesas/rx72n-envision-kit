...
wifi_err_t R_WIFI_ESP32_DeleteServerCertificate (uint32_t datatype)
{
	volatile int32_t timeout;
	volatile int32_t sended_length;
	int32_t current_send_length;
	...
	uint8_t *buff;
	uint32_t file_header_len;
	uint32_t write_total_len = 0x2000;
	uint32_t padding_len;
    ...
    if(0 == wifi_take_mutex(mutex_flag))
	{
		...
		ret = wifi_execute_at_command(WIFI_UART_COMMAND_PORT, g_wifi_uart[WIFI_UART_COMMAND_PORT].command_buff, 10000, WIFI_RETURN_ENUM_OK, WIFI_COMMAND_SET_SYSFALSH_WRITE_START, 0xff);
