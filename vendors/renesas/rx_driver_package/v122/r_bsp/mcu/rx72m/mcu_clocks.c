...
/* This macro runs or stops the Main-clock oscillator.
   ...
   4. ESC clock source is PPLL circuit. PPLL source is Main-clock oscillator.
   5. PHY clock source is PLL circuit. PLL source is Main-clock oscillator.
   6. PHY clock source is PPLL circuit. PPLL source is Main-clock oscillator.
   7. Clock output enable and clock output source is Main-clock oscillator.
   8. Clock output enable and clock output source is PLL circuit. PLL source is Main-clock oscillator.
   9. Clock output enable and clock output source is PPLL circuit. PPLL source is Main-clock oscillator.
 */
#if (BSP_CFG_CLOCK_SOURCE == 2) \
     || ((BSP_CFG_CLOCK_SOURCE == 4) && (BSP_CFG_PLL_SRC == 0)) \
     || ((BSP_CFG_USB_CLOCK_SOURCE == 3) && (BSP_CFG_PLL_SRC == 0)) \
     || ((BSP_CFG_ESC_CLOCK_SOURCE == 1) && (BSP_CFG_PLL_SRC == 0)) \
...
/* This macro runs or stops the PPLL circuit.
   ...
   2. ESC clock source is PPLL circuit.
   3. PHY clock source is PPLL circuit.
   4. Clock output enable and clock output source is PPLL circuit.
 */
#if (BSP_CFG_USB_CLOCK_SOURCE == 3) \
     || (BSP_CFG_ESC_CLOCK_SOURCE == 1) \
...
uint32_t get_iclk_freq_hz(void)
{
    ...
    switch (cksel)
    {
        ...
        case BSP_PRV_CKSEL_PLL:
            ...
            /* The RX72M have two possible sources for the PLL */
...
static void clock_source_select (void)
{
...
#if (BSP_CFG_PHY_CLOCK_SOURCE == 1) || (BSP_CFG_ESC_CLOCK_SOURCE == 1) || (BSP_CFG_USB_CLOCK_SOURCE == 3)
    /* initialize */
...
#if BSP_CFG_ESC_CLOCK_SOURCE == 1
    /* Set value of EPLLSEL bit. */
    tmp_packcr |= 0x0100;
#endif
    ...
    /* RX72M has a MEMWAIT register which controls the cycle waiting for access to code flash memory.
