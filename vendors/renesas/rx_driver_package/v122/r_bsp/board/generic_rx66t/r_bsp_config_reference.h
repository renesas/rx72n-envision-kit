...
/* NOTE:
   The default settings are the same as when using RSKRX66T.
...
/* ROM, RAM, and Data Flash Capacity.
   ...
   E            = 0xE             = 512KB/64KB/32KB
   A            = 0xA             = 256KB/64KB/32KB
*/
...
/* Clock source select (CKSEL).
   ...
   3 = Sub-Clock Oscillator                   (N/A for RX66T)
   4 = PLL Circuit
*/
...
/* Clock configuration options.
   ...
   ICLK is 160MHz, PCLKA is 80MHz, PCLKB is 40MHz, PCLKC is 160MHz, PCLKD is 40MHz, FCLK is 40MHz, USB Clock is 80MHz,
   and BCLK is 40MHz then the settings would be:

   BSP_CFG_XTAL_HZ  = 8000000
   BSP_CFG_PLL_DIV  = 1        (no division)
   BSP_CFG_PLL_MUL  = 20.0     (8MHz x 20.0 = 160MHz)
   BSP_CFG_ICK_DIV  = 1      : System Clock (ICLK)        = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_ICK_DIV)  = 160MHz
   ...
   BSP_CFG_PCKC_DIV = 1      : Peripheral Clock C (PCLKC) = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_PCKC_DIV) = 160MHz
   BSP_CFG_PCKD_DIV = 4      : Peripheral Clock D (PCLKD) = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_PCKD_DIV) = 40MHz
   BSP_CFG_FCK_DIV  = 4      : Flash IF Clock (FCLK)      = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_FCK_DIV)  = 40MHz
   BSP_CFG_BCK_DIV  = 4      : External Bus Clock (BCK)   = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_BCK_DIV)  = 40MHz
   BSP_CFG_UCK_DIV  = 2      : USB Clock (UCLK)           = 
                               (((BSP_CFG_XTAL_HZ/BSP_CFG_PLL_DIV) * BSP_CFG_PLL_MUL) / BSP_CFG_UCK_DIV)  = 80MHz
*/
...
*/
#define BSP_CFG_NONCACHEABLE_AREA0_ADDR      (0xFFFC0000)
...
/* Specifies the start address of non-cacheable area 1.
   ...
   The upper 10 bits are fixed at 1. The lower 4 bits are fixed at 0.
*/
#define BSP_CFG_NONCACHEABLE_AREA1_ADDR      (0xFFFC0000)
