...
volatile uint8_t R_BSP_EVENACCESS_SFR * const  sreg_ptrs[ADC_SST_REG_MAX+1] =
                      { &S12AD.ADSSTR0,     /* channel 0 */
                        ...
                        &S12AD.ADSSTRL,     /* channels 16 to 20 and 27 */
...
*
* Arguments    : unit -
*                    Ignored for rx23w. Pass 0 for this argument.
*                mode -
...
static adc_err_t adc_en_comparator_level0(adc_cmpwin_t  *p_cmpwin)
{
    ...
    if (true == p_cmpwin->windowa_enable) // COMPARATOR A
    {
        ...
        S12AD.ADCMPANSR1.WORD = (p_cmpwin->compare_mask>>16) & 0x081F; // Select channels to compare. Channels 16 to 20 and 27.
        S12AD.ADCMPLR1.WORD = (p_cmpwin->inside_window_mask>>16) & 0x081F;
...
static uint32_t adc_get_and_clr_cmpi_flags(void)
{
    ...
    /* Get flags for channels AN16 to AN20 and AN27 */
...
static adc_err_t adc_configure_scan(adc_ch_cfg_t   *p_config)
{
    ...
    /* Select the channels from among CH16 to CH20 and CH27 for conversion */
    S12AD.ADANSA1.WORD = (uint16_t) ((p_config->chan_mask >> 16) & 0x081F);      //CH16 to CH20 and CH27
    S12AD.ADANSB1.WORD = (uint16_t) ((p_config->chan_mask_groupb >> 16) & 0x081F);

    /* Select the channels for Addition/Averaging (CH16 to CH20 and CH27)*/
    S12AD.ADADS1.WORD  = (uint16_t) ((p_config->add_mask >> 16) & 0x081F);
...
* Description  : This is the RX23W implementation of R_ADC_Close().
*                This function ends any scan in progress, disables interrupts,
*                and removes power to the A/D peripheral.
* Arguments    : unit - This argument is ignored for the RX23W.
...
* Description  : This is the RX23W implementation of R_ADC_ReadAll().
...
#endif // #if defined (BSP_MCU_RX23W)
