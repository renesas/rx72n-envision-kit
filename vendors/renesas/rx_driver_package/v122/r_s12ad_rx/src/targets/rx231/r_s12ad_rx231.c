...
*                              Bug fix for ADC_CMD_EN_COMPARATOR_LEVEL and ADC_CMD_EN_COMPARATOR_WINDOW don't work.
...
*                              Corresponding to the notice for ADCSR.ADHSC rewrite sequence.
...
*
* Arguments    : unit -
*                    Ignored for rx231. Pass 0 for this argument.
*                mode -
...
static adc_err_t adc_en_comparator_level0(adc_cmpwin_t  *p_cmpwin)
{
    ...
    if (true == p_cmpwin->windowa_enable) // COMPARATOR A
    {
        ...
        S12AD.ADCMPANSR1.WORD = (p_cmpwin->compare_mask>>16) & 0x0FFFF; // Select channels to compare. 16 to 31.
        S12AD.ADCMPLR1.WORD = (p_cmpwin->inside_window_mask>>16) & 0x0FFFF;
...
static adc_err_t adc_configure_scan(adc_ch_cfg_t   *p_config)
{
    ...
    S12AD.ADANSA1.WORD = (uint16_t) ((p_config->chan_mask >> 16) & 0xFFFF);      //CH16 to CH31
...
* Description  : This is the RX231 implementation of R_ADC_Close().
*                This function ends any scan in progress, disables interrupts,
*                and removes power to the A/D peripheral.
* Arguments    : unit - This argument is ignored for the RX231.
...
* Description  : This is the RX231 implementation of R_ADC_ReadAll().
...
#endif // #if defined (BSP_MCU_RX231)
